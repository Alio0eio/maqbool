      ...
      <Tabs defaultValue="saved" className="w-full">
        <TabsList className="mb-4">
          ...
          <TabsTrigger value="alerts" className="gap-2">
            ...
            {activeAlertCount > 0 && (
              <span className="ml-1 text-xs font-mono bg-primary/10 text-primary rounded-full px-1.5">{activeAlertCount}</span>
            )}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="alerts" className="outline-none mt-0 space-y-4">
          <Card className="border-border shadow-sm">
            <CardContent className="p-6">
              <h3 className="text-base font-display font-semibold text-foreground mb-4">Create a new alert</h3>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div className="flex gap-2">
                  <select
                    ...
                    onChange={(e) => setForm(f => ({ ...f, frequency: e.target.value as JobAlert["frequency"] }))}
                    className="flex-1 h-9 rounded-md border border-input bg-white px-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring"
                  >
              ...
              <Card key={alert.id} className="border-border shadow-sm">
                <CardContent className="p-4 flex items-center gap-4">
                  ...
                  <button
                    ...
                    className="p-2 rounded-lg hover:bg-red-50 text-muted-foreground hover:text-destructive transition-all shrink-0"
                  >
