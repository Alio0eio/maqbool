    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-card p-4 rounded-xl border border-border shadow-sm flex flex-col lg:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <Input
            ...
            className="pl-9 bg-muted border-transparent focus:bg-card"
            value={searchTerm}
        ...
        <div className="flex gap-2">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" className="gap-2 bg-muted">
                <Filter className="w-4 h-4" />
                Skills
                {skillFilters.length > 0 && (
                  <Badge variant="secondary" className="ml-1 h-5 px-1 rounded font-mono">{skillFilters.length}</Badge>
                )}
              </Button>
            </DropdownMenuTrigger>
      ...
      <Tabs defaultValue="all" className="w-full">
        <TabsList className="mb-6">
          <TabsTrigger value="all">All Candidates <span className="font-mono ml-1">({filteredCandidates.length})</span></TabsTrigger>
          <TabsTrigger value="active" className="gap-2">
            Active in Pipeline <Badge variant="default" className="h-5 px-1 ml-1 rounded font-mono">{activeCandidates.length}</Badge>
          </TabsTrigger>
