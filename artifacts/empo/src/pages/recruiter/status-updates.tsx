...
const STATUS_TEMPLATES = [
  ...
  { label: 'Move to Interview', body: "Congratulations! We'd like to invite you to the next stage of our interview process for {role}. Please check your EMPO dashboard." },
        ...
        <Card className="lg:col-span-2 border-border shadow-sm flex flex-col overflow-hidden">
          <div className="px-4 py-4 border-b border-border shrink-0">
            <div className="flex items-center justify-between mb-2">
              <h2 className="font-display font-semibold text-foreground">Select Recipients</h2>
              <button
            ...
            <span className="text-xs text-muted-foreground font-mono">{selected.length} selected</span>
          </div>
        ...
        <div className="lg:col-span-3 flex flex-col gap-4">
          <Card className="border-border shadow-sm">
            <CardContent className="p-5">
              <h2 className="font-display font-semibold text-foreground mb-3">Status Template</h2>
              <div className="grid grid-cols-2 gap-2">
                {STATUS_TEMPLATES.map((t) => (
                  <button
                    ...
                    className={cn(
                      'p-3 rounded-lg border text-left text-xs font-medium transition-all',
                      template.label === t.label ? 'border-primary bg-primary/5 text-primary' : 'border-border hover:border-primary/40 text-muted-foreground'
                    )}
                  >
                    {t.label}
                  </button>
          ...
          <Card className="border-border shadow-sm flex-1 flex flex-col">
            <CardContent className="p-5 flex-1 flex flex-col">
              <h2 className="font-display font-semibold text-foreground mb-3">Message</h2>
              <Textarea
                value={message}
              ...
              />
              <p className="text-xs text-muted-foreground mt-2 font-mono">{'Use {role}, {candidate_name} for personalization.'}</p>
            </CardContent>
          </Card>
          ...
          <Button
            ...
            className={cn('h-12 gap-2', sent && 'bg-success hover:bg-success text-success-foreground')}
          >
            <Send size={16} />
