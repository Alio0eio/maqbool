...
function loadFromStorage(): { user: User | null; role: Role | null } {
  try {
    const role = localStorage.getItem('empo_role') as Role | null;
    const userId = localStorage.getItem('empo_user_id');
    if (role && userId) {
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signIn = useCallback((email: string, selectedRole: Role) => {
    ...
    localStorage.setItem('empo_role', selectedRole);
    localStorage.setItem('empo_user_id', String(found.id));
  }, []);

  const signOut = useCallback(() => {
    setUser(null);
    setRole(null);
    localStorage.removeItem('empo_role');
    localStorage.removeItem('empo_user_id');
  }, []);
