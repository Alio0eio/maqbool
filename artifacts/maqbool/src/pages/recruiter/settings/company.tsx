  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <PageHeader
        ...
        actions={
          <Button data-testid="btn-save" onClick={handleSave} className={saved ? 'bg-success text-success-foreground hover:bg-success/90' : undefined}>
      ...
      <div className="space-y-6">
        <Card>
          <CardHeader className="flex-row items-center gap-2.5 space-y-0">
            <Building2 className="w-[18px] h-[18px] text-primary shrink-0" />
            <div>
              <CardTitle className="text-[15px]">Company Profile</CardTitle>
              <CardDescription>Basic details candidates see on every job posting.</CardDescription>
            </div>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-[15px]">Benefits & Perks</CardTitle>
            <CardDescription>Shown to candidates as a quick summary of what you offer.</CardDescription>
          </CardHeader>
          <CardContent className="flex flex-wrap gap-2">
            {form.benefits.map(b => (
              <Badge key={b} variant="secondary" className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[12.5px] font-medium">
                {b}
                <button
                  type="button"
